...
@implementation BLEPeripheral {
    CFTimeInterval lastSendTime;
    NSMutableData *lastSendData;
}
...
//        _uuidString = [_activePeripheral.identifier.UUIDString getSubstringWithRange:NSMakeRange(_activePeripheral.identifier.UUIDString.length-36, 36)];
...
- (void)peripheral:(CBPeripheral *)peripheral didDiscoverCharacteristicsForService:(CBService *)service error:(NSError *)error{
    ...
    if (!error) {
        [[NSNotificationCenter defaultCenter] postNotificationName:@"aaaaaaaaaasdfasdf" object:service];
...
- (void)peripheral:(CBPeripheral *)peripheral didUpdateNotificationStateForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error{
    ...
    NSLog(@"peripheralName :%@",peripheral.name);
      [[NSNotificationCenter defaultCenter] postNotificationName:@"nCBCentralStateChange"  object:[NSString stringWithFormat:@"didconnect"]];
}
...
//    [self writeValue:_activePeripheral characteristic:_transDataCharateristic data:data];
    ...
    /* 20190603 修改
     发送命令的间隔在 100 毫秒以外 1 记录上次的世界
     */
    if (data.length <= 0) {
        return;
    }
    CFTimeInterval currentTime = CFAbsoluteTimeGetCurrent();
    if (lastSendTime == 0 || currentTime - lastSendTime >  0.1 || ![lastSendData isEqualToData:data]) {
        [self writeValue:_activePeripheral characteristic:_transDataCharateristic data:data];
        lastSendTime =  CFAbsoluteTimeGetCurrent();
        lastSendData = [NSMutableData dataWithData:data];
        NSLog(@"+++++正常发送指令");
    }else {
        NSLog(@"+++++发送太快了，请慢点发送指令 ");
        [[NSNotificationCenter defaultCenter] postNotificationName:@"SEND_CMD_FAST" object:nil];
    }
...
-(void)dealloc{
    [[NSNotificationCenter defaultCenter] removeObserver:self name:@"CBUpdataShowStringBuffer" object:nil];
}
@end
